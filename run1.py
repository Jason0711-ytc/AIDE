...
# GPU selection:
# - If you already set CUDA_VISIBLE_DEVICES outside, we won't override it.
# - Otherwise you can select a card via: MMTL_GPU=1 python run.py
if "CUDA_VISIBLE_DEVICES" not in os.environ:
    selected_gpu = os.environ.get("MMTL_GPU")
    if selected_gpu is not None and selected_gpu.strip() != "":
        os.environ["CUDA_VISIBLE_DEVICES"] = selected_gpu.strip()
import math
...
def uncertainty_weighted_loss(model, losses):
    """
    Uncertainty-weighted multi-task loss:
        sum_i exp(-s_i) * L_i + s_i
    where s_i are learnable log variances (log_vars).

    Args:
        model: TotalNet or nn.DataParallel(TotalNet)
        losses: list/tuple of 4 scalar loss tensors [L1, L2, L3, L4]
    Returns:
        total_loss (scalar tensor), weights (detached tensor of shape [4])
    """
    if isinstance(model, nn.DataParallel):
        log_vars = model.module.log_vars
    else:
        log_vars = model.log_vars

    loss_vec = torch.stack(list(losses))  # [4]
    precision = torch.exp(-log_vars)      # [4]

    total = torch.sum(precision * loss_vec + log_vars)
...
class CarDataset(Dataset):
    ...
    def __init__(self, csv_file, transform=None, data_root: str = ""):

        self.path = pd.read_csv(csv_file)
        # self.path='/root/'+self.path
        self.transform = transform
        self.data_root = data_root or os.environ.get("AIDE_ROOT", "")
    ...
    def __getitem__(self, idx):
        ...
        if self.data_root and not os.path.isabs(frames_path):
            frames_path = os.path.join(self.data_root, frames_path)
        if self.data_root and not os.path.isabs(label_path):
    ...
    def load_frames(self, file_dir, pose_list):
        ...
        incar_dir = os.path.join(file_dir, 'incarframes')
        front_dir = os.path.join(file_dir, 'frontframes')
        left_dir = os.path.join(file_dir, 'leftframes')
        right_dir = os.path.join(file_dir, 'rightframes')
        face_dir = os.path.join(file_dir, 'face')
        body_dir = os.path.join(file_dir, 'body')

        def _crop_bbox(img, bbox_xywh):
            if img is None:
                return None
            h, w = img.shape[:2]
        ...
        def _select_indices(max_raw_frames: int):
            indices = []
            for i in range(max_raw_frames):
                if not i == 0 and not i % 3 == 2:
                    continue
                indices.append(i)
            return set(indices)

        max_raw_frames = min(45, len(pose_list))
        selected = _select_indices(max_raw_frames)

        buffer, buffer_front, buffer_left, buffer_right, buffer_face, buffer_body = [], [], [], [], [], []
        posture_list, gesture_list = [], []

        # Prefer jpg frames if present (but they cost a lot of disk)
        has_frame_dirs = all(os.path.isdir(p) for p in [incar_dir, front_dir, left_dir, right_dir])
        has_face_body_dirs = os.path.isdir(face_dir) and os.path.isdir(body_dir)

        if has_frame_dirs:
            frames = [os.path.join(incar_dir, img) for img in os.listdir(incar_dir) if img.endswith('.jpg')]
            front_frames = [os.path.join(front_dir, img) for img in os.listdir(front_dir) if img.endswith('.jpg')]
            left_frames = [os.path.join(left_dir, img) for img in os.listdir(left_dir) if img.endswith('.jpg')]
            right_frames = [os.path.join(right_dir, img) for img in os.listdir(right_dir) if img.endswith('.jpg')]
            ...
            max_raw_frames = min(max_raw_frames, len(frames), len(front_frames), len(left_frames), len(right_frames))

            face_frames = []
            body_frames = []
            if has_face_body_dirs:
                face_frames = [os.path.join(face_dir, img) for img in os.listdir(face_dir) if img.endswith('.jpg')]
                body_frames = [os.path.join(body_dir, img) for img in os.listdir(body_dir) if img.endswith('.jpg')]
                ...
                if len(face_frames) != 45 and len(face_frames) > 0:
                    face_frames.extend([face_frames[-1]] * (45 - len(face_frames)))
                if len(body_frames) != 45 and len(body_frames) > 0:
            ...
            for i in range(max_raw_frames):
                ...
                if has_face_body_dirs and i < len(face_frames) and i < len(body_frames):
                    img_face = cv2.imread(face_frames[i])
                    img_body = cv2.imread(body_frames[i])
                else:
                    face_bbox = pose_list[i]['result'][0].get('face_bbox', [0, 0, 1, 1])
                    body_bbox = pose_list[i]['result'][0].get('bbox', [0, 0, img.shape[1], img.shape[0]])
                    img_face = _crop_bbox(img, face_bbox)
                    img_body = _crop_bbox(img, body_bbox)
                ...
                try:
                    ...
                    img_face = cv2.resize(img_body, (self.face_width, self.face_height))
            ...
            # Fallback: decode from mp4 on-the-fly (saves disk; costs CPU)
            incar_mp4 = os.path.join(file_dir, 'incar.mp4')
            front_mp4 = os.path.join(file_dir, 'front.mp4')
            left_mp4 = os.path.join(file_dir, 'left.mp4')
            right_mp4 = os.path.join(file_dir, 'right.mp4')

            missing = [p for p in [incar_mp4, front_mp4, left_mp4, right_mp4] if not os.path.exists(p)]
            if missing:
                raise FileNotFoundError(f"Missing frame folders and mp4 videos under {file_dir}: {missing}")

            caps = [
                cv2.VideoCapture(incar_mp4),
                cv2.VideoCapture(front_mp4),
                cv2.VideoCapture(left_mp4),
                cv2.VideoCapture(right_mp4),
            ]
            try:
                for i in range(max_raw_frames):
                    ...
                    img, front_img, left_img, right_img = [frame for (_, frame) in rets_frames]
...
def _resolve_existing_path(path_str: Optional[str], *, label: str) -> Optional[str]:
    if not path_str:
        return None
...
def _resolve_csv_path(env_key: str, default_filename: str) -> str:
    env_value = os.environ.get(env_key)
    resolved_env = _resolve_existing_path(env_value, label=env_key)
    if resolved_env:
        return resolved_env

    # Common launch patterns: from repo root, from arbitrary CWD, from script dir.
    script_dir = Path(__file__).resolve().parent
    workspace_dir = script_dir.parent
    candidates = [
        Path.cwd() / default_filename,
        script_dir / default_filename,
        workspace_dir / default_filename,
        workspace_dir / "AIDE-main" / default_filename,
    ]
    ...
    raise FileNotFoundError(
        ...
        f"Tried env var {env_key}={env_value!r} (not found) and searched:\n{searched}\n\n"
        f"Fix: set env vars, e.g.:\n"
        f"  export {env_key}=/path/to/{default_filename}\n"
        f"  export AIDE_ROOT=/path/to/dir_containing_AIDE_Dataset"
    )


train_csv = _resolve_csv_path('AIDE_TRAIN_CSV', 'training.csv')
val_csv = _resolve_csv_path('AIDE_VAL_CSV', 'validation.csv')
test_csv = _resolve_csv_path('AIDE_TEST_CSV', 'testing.csv')


def _resolve_aide_root(aide_root_env: str) -> str:
    env_candidate = Path(aide_root_env).expanduser() if aide_root_env else None
    if env_candidate:
        if (env_candidate / 'AIDE_Dataset').is_dir():
            return str(env_candidate)
        if env_candidate.is_dir() and env_candidate.name == 'AIDE_Dataset':
            return str(env_candidate.parent)
    ...
    candidates = [
        ...
        script_dir,
        workspace_dir,
        workspace_dir / 'datasets' / 'AIDE' / 'extracted',
        workspace_dir / 'datasets' / 'AIDE',
        workspace_dir / 'data',
    ]
    for candidate in candidates:
        if (candidate / 'AIDE_Dataset').is_dir():
            return str(candidate)
    return ''


data_root = r"/data/tianchengyu/datasets"
...
def _precheck_csv_paths(csv_path: str, aide_root: str, *, n: int = 5) -> None:
    ...
    if df.shape[1] < 2:
        raise ValueError(f"CSV {csv_path} must have at least 2 columns: frames_path,label_path")
    for i in range(min(n, len(df))):
        frames_path, label_path = df.iloc[i, 0], df.iloc[i, 1]
        frames_path = str(frames_path)
        label_path = str(label_path)
        frames_abs = frames_path if os.path.isabs(frames_path) else str(Path(aide_root) / frames_path)
        label_abs = label_path if os.path.isabs(label_path) else str(Path(aide_root) / label_path)
        if not Path(label_abs).exists():
            raise FileNotFoundError(
                f"Label file missing for CSV row {i}: {label_path} -> {label_abs}\n"
                f"Check that AIDE_ROOT points to the directory containing AIDE_Dataset. Current AIDE_ROOT={aide_root!r}"
            )
        if not Path(frames_abs).exists():
            raise FileNotFoundError(
                f"Frames directory missing for CSV row {i}: {frames_path} -> {frames_abs}\n"
                f"Check that AIDE_ROOT points to the directory containing AIDE_Dataset. Current AIDE_ROOT={aide_root!r}"
            )


_precheck_csv_paths(train_csv, data_root, n=3)
_precheck_csv_paths(val_csv, data_root, n=3)
_precheck_csv_paths(test_csv, data_root, n=3)

train_batch_size = int(os.environ.get('MMTL_BATCH_TRAIN', '24'))
val_batch_size = int(os.environ.get('MMTL_BATCH_VAL', '64'))
test_batch_size = int(os.environ.get('MMTL_BATCH_TEST', '24'))
num_workers = int(os.environ.get('MMTL_NUM_WORKERS', '4'))
test_num_workers = int(os.environ.get('MMTL_TEST_NUM_WORKERS', '0'))

train_dataset = CarDataset(csv_file=train_csv, data_root=data_root)  # 'training.csv'
...
class _NCHWBiLevelRoutingAttention(nn.Module):
    def __init__(self, bra: nn.Module):
        super().__init__()
        self.bra = bra

    def forward(self, x: torch.Tensor) -> torch.Tensor:
        # BiLevelRoutingAttention expects NCHW (it converts to NHWC internally)
        return self.bra(x)
...
class TotalNet(nn.Module):
    def __init__(self):
        ...
        # NOTE: Original code calls self.net1..self.net4 in forward() but they were
        # never defined, causing training to crash. Keep minimal behavior (no-op).
        ...
        self.net1 = _NCHWBiLevelRoutingAttention(self.marnet1)
        self.net2 = _NCHWBiLevelRoutingAttention(self.marnet2)
        self.net3 = _NCHWBiLevelRoutingAttention(self.marnet3)
        self.net4 = _NCHWBiLevelRoutingAttention(self.marnet4)
        ...
        # 4 tasks: emotion, behavior, scene context, vehicle context
...
checkpoint_dir = os.environ.get('MMTL_OUTPUT_DIR', './mmtl_outputs')
...
# Posture Skeleton Keypoint 3 (C)×16 (F)×26 (K)×1 (P)
            ...
            gesture = gesture.to(device, non_blocking=True)
            ...
            loss, task_weights = uncertainty_weighted_loss(model, [loss1, loss2, loss3, loss4])
            # print(loss)
...
# Posture Skeleton Keypoint 3 (C)×16 (F)×26 (K)×1 (P)
                ...
                with open(file=os.path.join(checkpoint_dir, "val_CNNTrans_basic_v5.txt"), mode="a+") as f:
                    f.write("Epoch: %d, Subepoch: %d, Loss: %f, batch_size: %d, total_acc1: %f,total_acc2: %f, total_acc3: %f, total_acc4: %f, \
        ...
        print(f"===== Epoch {epoch + 1}/{EPOCHS} done in {time.time() - epoch_start_time:.1f}s =====\n", flush=True)


        # 保存最佳模型(将当前模型的权重保存到best_model.pt文件中)
        best_path = os.path.join(checkpoint_dir, 'best_model_CNNTrans_basic_v5.pt')
        if is_best:
            with open(file=os.path.join(checkpoint_dir, "val_CNNTrans_basic_v5.txt"), mode="w") as f:
                    f.write("Epoch: %d, Subepoch: %d, Loss: %f, batch_size: %d, total_acc1: %f,total_acc2: %f, total_acc3: %f, total_acc4: %f, \
                            avgf11: %f, avgf12: %f, avgf13: %f, avgf14: %f\n"\
                     %(epoch, subepoch1, val_losses, M, val_acc1.getacc(), val_acc2.getacc(), val_acc3.getacc(), val_acc4.getacc(),avgf11, avgf12, avgf13, avgf14))
...
def test(use_cuda=True, batch_size=16, model_name: str = ""):
    ...
    if not data_root:
        raise FileNotFoundError(
            "AIDE dataset root not found for test(). Set AIDE_ROOT to the directory containing AIDE_Dataset.\n"
            "Example:\n"
            "  export AIDE_ROOT=/data1/yanjing/datasets/AIDE/extracted"
        )
    test_dataset = CarDataset(csv_file=test_csv, data_root=data_root)
...
# Posture Skeleton Keypoint 3 (C)×16 (F)×26 (K)×1 (P)
            ...
            with open(file=os.path.join(checkpoint_dir, "test_CNNTrans_basic_v5.txt"), mode="a+") as f:
...
if __name__ == "__main__":
    mode = os.environ.get('MMTL_MODE', 'train').strip().lower()
    cuda = os.environ.get('MMTL_CUDA', '1').strip() not in ('0', 'false', 'no')
    epochs = int(os.environ.get('MMTL_EPOCHS', '20')) if mode == 'train' else None

    # Write a log file while still streaming to the terminal (no nohup needed).
    # Can be disabled via: MMTL_LOG=0
    log_enabled = os.environ.get('MMTL_LOG', '1').strip() not in ('0', 'false', 'no')
    log_path = None
    if log_enabled:
        os.makedirs(checkpoint_dir, exist_ok=True)
        ts = datetime.datetime.now().strftime('%Y%m%d_%H%M%S')
        log_path = os.environ.get('MMTL_LOG_PATH', os.path.join(checkpoint_dir, f"{mode}_{ts}.log"))
    ...
    if mode == 'train':
        print("running...", "mode=", mode, "cuda=", cuda, "epochs=", epochs, flush=True)
    else:
        print("running...", "mode=", mode, "cuda=", cuda, flush=True)
    if log_path:
        print(f"logging to: {log_path}", flush=True)
    if mode == 'train':
        print(f"Total epochs to run: {epochs}", flush=True)

    if mode == "train":
        main(use_cuda=cuda, EPOCHS=int(epochs), batch_size=train_batch_size)
        print("main mode...")
    elif mode == "test":
        test(use_cuda=cuda, batch_size=test_batch_size)
        print("test mode...")
    else:
        raise ValueError(f"Unknown MMTL_MODE: {mode}")